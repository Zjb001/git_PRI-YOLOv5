...
def train(hyp,  # path/to/hyp.yaml or hyp dictionary
    ...
    save_dir, epochs, batch_size, weights, single_cls, evolve, data, cfg, resume, noval, nosave, workers, = \
        ...
        opt.resume, opt.noval, opt.nosave, opt.workers  # data .yaml; weight .pt;
    ...
    is_coco = data.endswith('coco.yaml') and nc == 9  # COCO dataset
    ...
    # ---------------Model---------------before this，prepare for all path，logger，show during train
    ...
    if pretrained:
        ...
        model = Model(cfg, ch=3, nc=9, anchors=hyp.get('anchors')).to(device)  # create
    ...
    # Trainloader
    ''' imgs,targets,datasets.labels√ 得改成服务器版本；暂时把图片拼接关了吃不消'''
    # 把batch_size // WORLD_SIZE改为图片总个数
    ...
    if RANK in [-1, 0]:
        ...
        val_loader, dataset1 = create_dataloader(val_path, imgsz, batch_size // WORLD_SIZE, gs, single_cls,
    ...
    compute_loss1 = ComputeLoss1(model)
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:
            ...
            with amp.autocast(enabled=cuda):
                ...
                # computer loss
                # temp_targets = torch.full_like(targets,0.0)
                # temp_targets[:,0]=batch_size*(i)
                # targets = targets+temp_targets
                indexn = batch_size*(i)

                loss, loss_items = compute_loss(pred, targets.to(device), epoch, i, imgs)
        ...
        if RANK in [-1, 0]:
            ...
            if not noval or final_epoch:  # Calculate mAP
                results, maps, _ = val.run(data_dict,
                                           ...
                                           epoch=epoch)  # results：mp, mr, map50, mapn, *(loss.cpu() / len(dataloader)).tolist()

            # Update best mAP
            fi = fitness(np.array(results).reshape(1, -1))  # weighted combination of [P, R, mAP@.5, mAP@.5-.95]
            if fi > best_fitness:
                best_fitness = fi
            loggers.on_train_val_end(mloss, results, lr, epoch, best_fitness, fi)  # tensor4, tuple8, list3, int,array,array
...
def parse_opt(known=False):
    ...
    parser.add_argument('--project', default='runs/train', help='save to project/name')  # save path
...
if __name__ == "__main__":
    ...
    from utils.loss_copy2 import ComputeLoss1
    ...
    wandb.init(project="yolov5_master", entity="zjb")
    # wandb = None
