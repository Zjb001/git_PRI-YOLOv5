...
class FocalLoss(nn.Module):
    ...
    def __init__(self, loss_fcn, gamma=1.5, alpha=0.25):
        ...
        self.loss_fcn = loss_fcn  # must be nn.BCEWithLogitsLoss()  # Multi-classification cross-entropy loss function
        self.gamma = gamma
        self.alpha = alpha
        self.reduction = loss_fcn.reduction
        self.loss_fcn.reduction = 'none'  # required to apply FL to each element
...
class ComputeLoss:
    ...
    def build_targets(self, p, targets):
        ...
        targets[:, :, [0,1,2,3,4,5,6,7,8]] = targets[:, :, [0,1,2,3,4,5,8,6,7]]
    ...
    def ssim1(self,im1,im2):
        ...
        ssim_map = ((2 * ux*uy + C1) * (2 * oxy + C2)) / ((np.square(ux) + np.square(uy) + C1) * (np.square(ox) + np.square(oy) + C2))
        return ssim_map
    ...
    def cal_index(self, img, tem):  # img:numpy;tem:list xywh
        ...
        temp_list=[ares1,ares2,ares3,ares4,ares5,ares6,ares7,ares8,ares9]  # float32
        over_list=[]
        ...
        zone9 = over_list[8]
        '''
        zone1, zone2, zone3, zone4 = color.rgb2lab(ares1), color.rgb2lab(ares2), color.rgb2lab(
            ares3), color.rgb2lab(ares4)
        zone5, zone6, zone7, zone8 = color.rgb2lab(ares5), color.rgb2lab(ares6), color.rgb2lab(
            ares7), color.rgb2lab(ares8)
        zone9 = color.rgb2lab(ares9)
        '''
